function va=GetValue(im,y)
% getValue computes the value of im at position y using 
% linear interpolation. This functions differes from getLValue
% by not considering im to be wrapped between -pi and pi.
...
at=-(im(m(1)+1,m(2))-im(m(1),m(2)))-(im(m(1),m(2)+1)-im(m(1),m(2)))+(im(m(1)+1,m(2)+1)-im(m(1),m(2)));
bt=(im(m(1),m(2)+1)-im(m(1),m(2)));
ct=(im(m(1)+1,m(2))-im(m(1),m(2)));
va=(at*dm(1)*dm(2)+bt*dm(2)+ct*dm(1)+im(m(1),m(2)));
